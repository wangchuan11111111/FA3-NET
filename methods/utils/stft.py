...
def calculate_spectral_centroid(spectrogram):
    """
    Calculate the spectral centroid for each frame in a spectrogram.

    Parameters:
    spectrogram (torch.Tensor): STFT spectrogram of shape (batch_size, num_channels, time_steps, freq_bins)
    sample_rate (int): Sampling rate of the audio signal
    n_fft (int): Number of FFT components
    hop_length (int): Hop length for STFT

    Returns:
    torch.Tensor: Spectral centroid of shape (batch_size, num_channels, time_steps)
    """
    # Frequency values for each FFT bin
    freqs = torch.linspace(0, 24000 // 2 + 1, spectrogram.size(3), device=spectrogram.device)

    # Reshape freqs to match spectrogram shape: [freq_bins] -> [1, 1, 1, freq_bins]
    freqs = freqs.view(1, 1, 1, -1)

    # Calculate the weighted sum of frequencies (numerator)
    numerator = torch.sum(spectrogram * freqs, dim=3)

    # Calculate the sum of magnitudes (denominator)
    denominator = torch.sum(spectrogram, dim=3)

    # Compute the spectral centroid
    centroid = numerator / (denominator + 1e-6)  # Avoid division by zero
