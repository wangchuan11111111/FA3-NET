...
def time_mask(spec, T=2, num_masks=5, replace_with_zero=False):
    cloned = spec.clone()
...
def frequency_mask(spec, F=2, num_masks=2, replace_with_zero=False):
    cloned = spec.clone()
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, args, cfg, dataset, af_extractor, valid_set, model, optimizer, losses, metrics):
        ...
        fn_scalar = '{}_nfft{}_hop{}_mel{}_{}.h5'.format(cfg['data']['audio_feature'], \
                                                         cfg_data['nfft'], cfg_data['hoplen'], cfg_data['n_mels'],
                                                         dataset_name)
    ...
    def train_step(self, batch_sample, epoch_it):
        ...
        if self.af_extractor:
            ...
            # batch_x = frequency_mask(batch_x)
    ...
    def validate_step(self, generator=None, max_batch_num=None, valid_type='train', epoch_it=0):
        ...
        if valid_type == 'train':
            ...
            for _, path in enumerate(self.valid_paths_dict):
                ...
                pred_dcase_format = track_to_dcase_format(pred_sed[frame_ind:frame_ind + loc_frames],
                                                          pred_doa[frame_ind:frame_ind + loc_frames])
