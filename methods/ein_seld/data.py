...
def time_mask(spec, T=4, num_masks=10, replace_with_zero=False):
    cloned = spec.copy()
...
def frequency_mask(spec, F=2, num_masks=4, replace_with_zero=False):
    cloned = spec.copy()
...
def collate_fn_train(batch_dict):
    """
    Merges a list of samples to form a mini-batch
    Pin memory for customized dataset
    """
    return PinMemCustomBatch_train(batch_dict)
