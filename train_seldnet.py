...
def main(argv):
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        model = seldnet_model.ConvConformer().to(device)
        ...
        print('SED metrics: Error rate: {:0.3f} {}, F-score: {:0.3f} {}'.format(test_ER[0]  if use_jackknife else test_ER, '[{:0.3f}, {:0.3f}]'.format(test_ER[1][0], test_ER[1][1]) if use_jackknife else '', 100* test_F[0]  if use_jackknife else 100* test_F, '[{:0.3f}, {:0.3f}]'.format(100* test_F[1][0], 100* test_F[1][1]) if use_jackknife else ''))
        print('DOA metrics: Localization error: {:0.3f} {}, Localization Recall: {:0.3f} {}'.format(test_LE[0] if use_jackknife else test_LE, '[{:0.3f} , {:0.3f}]'.format(test_LE[1][0], test_LE[1][1]) if use_jackknife else '', 100*test_LR[0]  if use_jackknife else 100*test_LR,'[{:0.3f}, {:0.3f}]'.format(100*test_LR[1][0], 100*test_LR[1][1]) if use_jackknife else ''))
